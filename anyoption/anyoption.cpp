...
{
	file_delimiter_char = _delimiter;
}
...
AnyOption::processLine( char *theline, int length  )
{
        ...
        if( *cursor == file_delimiter_char || *(cursor+length-1) == file_delimiter_char ){
                justValue( pline );/* line with start/end delimiter */
        }else{
//                for( int i = 1 ; i < length-1 && !found ; i++){/* delimiter */
                for( int i = 0 ; i < length && !found ; i++){/* delimiter */
                        if( *cursor == file_delimiter_char ){
//                            *(cursor-1) = nullterminate; /* two strings */
                            *(cursor) = nullterminate; /* two strings */
                                found = true;
